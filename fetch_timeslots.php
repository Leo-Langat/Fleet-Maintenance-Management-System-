...
// Get task details from the maintenance_tasks table
$query = "SELECT task_id, estimated_time FROM maintenance_tasks WHERE task_id = ?";
...
foreach ($service_centers as $service_center_id => $service_center) {
    ...
    foreach ($service_center['timeslots'] as $timeslot) {
        ...
        echo "<div class='timeslot {$status_class}' data-center-id='{$service_center_id}' data-start-time='{$timeslot['start_time']}' data-end-time='{$timeslot['end_time']}'>{$timeslot['start_time']} to {$timeslot['end_time']}</div>";
    }
    echo "</div></div>";
}
?>
